        ...
        function onMessage(message, cb) {
            ...
            switch (type) {
                ...
                case 'call':
                    ...
                    // if there are already 2 connections inform others
                    // that you want to joined
                    if(members.length > 0)
                        type = 'joined-call';
                    ...
                    // emit to others
                    socket.broadcast.to(to).send(data);

                    if(rooms[to].members.length > 0) {
                        // when you joined inform also self of members
                        // emit to self
                        data.type = 'wait-calls';
